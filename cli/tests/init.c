...
static const uint8_t DATA[] = {
  ...
  // func w/local.get out of bounds (fail, ofs: 1123, len: 27)
  0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00,
  0x01, 0x05, 0x01, 0x60, 0x00, 0x01, 0x7F, 0x03,
  0x02, 0x01, 0x00, 0x0A, 0x06, 0x01, 0x04, 0x00,
  0x20, 0x00, 0x0B,

  // func w/local.tee out of bounds (fail, ofs: 1150, len: 29)
  0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00,
  0x01, 0x05, 0x01, 0x60, 0x00, 0x01, 0x7F, 0x03,
  0x02, 0x01, 0x00, 0x0A, 0x08, 0x01, 0x06, 0x00,
  0x41, 0x00, 0x22, 0x00, 0x0B,

  // func w/local.set out of bounds (fail, ofs: 1179, len: 31)
  0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00,
  0x01, 0x05, 0x01, 0x60, 0x00, 0x01, 0x7F, 0x03,
  0x02, 0x01, 0x00, 0x0A, 0x0A, 0x01, 0x08, 0x00,
  0x41, 0x00, 0x21, 0x00, 0x41, 0x00, 0x0B,
};

static const init_test_t TESTS[] = {
  ...
  { "func w/local.get out of bounds",     false, 1123,   27 },
  { "func w/local.tee out of bounds",     false, 1150,   29 },
  { "func w/local.set out of bounds",     false, 1179,   31 },
};
...
void test_init_mods(
  ...
  for (size_t i = 0; i < LEN(TESTS); i++) {
    ...
    const init_test_t test = TESTS[i];
    const pwasm_buf_t buf = { DATA + test.ofs, test.len };

    // run test, get result
    pwasm_mod_t mod;
    const size_t len = pwasm_mod_init(&mem_ctx, &mod, buf);

    // check test result
    if ((len > 0) == test.want) {
      cli_test_pass(test_ctx, cli_test, test.name);
    } else {
      cli_test_fail(test_ctx, cli_test, test.name);
    }
